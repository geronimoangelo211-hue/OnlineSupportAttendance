...
if (_needsSave) localStorage.setItem('students', JSON.stringify(_studentsInit));
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // NEW: Check for Incognito before loading the Student Portal
    ...
    if (adminCanvas) {
        adminCanvas.addEventListener('click', generateAdminCaptcha);
    }
...
setInterval(() => {
    ...
    let updatedTimeOut = checkAndApplyAutoTimeOut(); // NEW CHECK
...
async function switchView(viewId) {
    ...
    if (viewId === 'student-view') {
        // NEW: Check Incognito every time they switch back to the portal
        const isPrivate = await isIncognito();
        if (isPrivate) {
            document.getElementById('turn-in-form').style.display = 'none';
            document.getElementById('locked-screen').style.display = 'none';
...
async function loginAdmin() {
    const userCaptcha = document.getElementById('admin-captcha-input').value;
    if (userCaptcha !== currentAdminCaptchaString) {
        showMessage('login-message', 'CAPTCHA Failed. Form erased.', 'error');
    ...
    if (!user || !pass) {
        ...
        generateAdminCaptcha(); // Reset on blank submission
        return;
    }
    ...
    try {
        ...
        if (contentType && contentType.includes("text/html")) {
            console.error("Ngrok HTML Warning Page Intercepted the Request.");
            showMessage('login-message', 'ACTION REQUIRED: Open your Ngrok link in a new tab and click "Visit Site" first.', 'error');
            return;
        }
        ...
        if (response.ok && data.success) {
            ...
            sessionStorage.setItem('currentAdminSec', 'sec-dashboard');
            
            switchView('admin-dashboard-view');
            const navItems = document.querySelectorAll('.admin-nav-item');
            if (navItems.length > 0) {
                switchAdminSection('sec-dashboard', navItems[0]);
            }
        } else {
            showMessage('login-message', data.message || 'Invalid username or password.', 'error');
            document.getElementById('admin-pass').value = ''; 
            generateAdminCaptcha(); 
        }
    } catch (error) {
...
function fetchAdminAccounts() {
    ...
    .then(async res => {
        ...
        if (contentType && contentType.includes("text/html")) {
            throw new Error("Ngrok HTML Warning Intercepted the Request.");
        }
        return res.json();
    })
    .then(data => {
        ...
        data.forEach(user => {
            ...
            let delBtn = '';
            // Changed from 'HeadAcc' to 'MainHeadAcc'
            if(user !== 'MainHeadAcc') { 
    ...
    .catch(err => {
        ...
        const list = document.getElementById('admin-accounts-list');
        if (list) list.innerHTML = `<li style="color: var(--error); padding: 10px; text-align: center;">Unable to load. Open Ngrok link in a new tab first.</li>`;
    });
}
...
async function createAdminAccount() {
    ...
    try {
        ...
        const contentType = response.headers.get("content-type");
        if (contentType && contentType.includes("text/html")) {
            showMessage('acc-message', 'Please open the Ngrok link in a new tab first.', 'error');
            return;
        }
        ...
        showMessage('acc-message', 'Server error. Is Ngrok running?', 'error');
    }
}
...
function createStudent() {
    ...
    showMessage('admin-message', 'Student created successfully!', 'success');
...
function updateStudentGC() {
    ...
    showMessage('edit-gc-message', 'GC Handle updated successfully!', 'success');
...
function renderStudents() {
    ...
    // Safely check if the input exists before reading its value
    ...
    filteredStudents.forEach(student => {
        const li = document.createElement('li');
        const safeId = student.id.replace(/'/g, "\\'"); // Ensures IDs with spaces/apostrophes don't break the button
        
        let gcTag = '';
        if (student.gcHandle) {
            gcTag = `<span class="gc-tag">${student.gcHandle}</span>`;
        }
        ...
        li.innerHTML = `
            ...
            <button onclick="viewPerformance('${safeId}')" style="background: rgba(var(--accent-rgb), 0.1); color: var(--accent); padding: 6px 15px; border-radius: 4px; font-size: 11px; border: 1px solid var(--accent); letter-spacing: 1px; cursor: pointer; white-space: nowrap;">
                VIEW PERFORMANCE
            </button>
        `;
        list.appendChild(li);
...
async function resetStudentUI() {
    // NEW: Make sure unlocking the UI respects Incognito
    const isPrivate = await isIncognito();
    if (isPrivate) {
        document.getElementById('turn-in-form').style.display = 'none';
        document.getElementById('locked-screen').style.display = 'none';
...
function handleTimeIn() {
    if (!isCaptchaSolved) {
        ...
        initSliderCaptcha();
        checkDeviceLock(); // Restore input if locked
        return;
    }
    ...
    // NEW: Lock the device to this student
...
function renderSchedule() {
    ...
    // 3. BULLETPROOF SORTING
    filteredStudents.sort((a, b) => {
        // Clean strings to prevent spacing/casing issues from breaking the sort
        const nameA = (a.name || '').toLowerCase().trim();
        const nameB = (b.name || '').toLowerCase().trim();
        const idA = (a.id || '').toString().trim();
        const idB = (b.id || '').toString().trim();
        
        // Push students with no tags to the very bottom using 'zzzz'
        ...
        if (sortVal === 'TAG_ASC') {
            if (tagA === tagB) {
                // If tags are identical, sort them alphabetically by name
                return nameA < nameB ? -1 : (nameA > nameB ? 1 : 0); 
            }
...
function devClearLogs() {
    if(confirm("This will permanently delete ALL attendance logs. Perfect for testing. Continue?")) {
        localStorage.setItem('attendanceLogs', JSON.stringify([]));
        if (document.getElementById('admin-dashboard-view').classList.contains('active')) {
            renderLogs();
            renderHistoryView();
        ...
        showMessage('dev-message', 'All logs cleared. Clean slate!', 'success');
    }
}
...
function renderMainDashboard() {
    try {
        ...
        if (barChartEl && barLabelsEl) {
            ...
            // Set the absolute chart limit to 100 students
            let maxPresent = 100; 
            let weeklyData = [];
            ...
            weeklyData.forEach(data => {
                ...
                // Prevent the bar from breaking out of the container if attendance somehow exceeds 100
                if (heightPct > 100) heightPct = 100; 
                // Keep a minimum visual height of 5% for empty days so a tiny sliver is always visible
                if (heightPct < 5) heightPct = 5; 
                
                barChartEl.innerHTML += `
                    <!-- Added height: 100% to this wrapper div so the bar knows how tall it can grow -->
...
function checkAndApplyAutoTimeOut() {
    ...
    // Look through all unique dates in the logs
    const uniqueDates = [...new Set(logs.map(l => l.date))];

    uniqueDates.forEach(dateStr => {
        // As soon as the day changes (past midnight), close out previous days
        if (dateStr !== todayStr) {
            const dayLogs = logs.filter(l => l.date === dateStr);
            const studentIds = [...new Set(dayLogs.map(l => l.id))];
            ...
            studentIds.forEach(id => {
                ...
                // If they timed in but never timed out that day, force a late timeout
                if (hasTimeIn && !hasTimeOut) {
                    ...
                    logs.push({
                        ...
                        time: '11:59:59 PM', // Logs them out at the very end of their shift day
                        date: dateStr,
                        details: {
    ...
    if (updated) {
        localStorage.setItem('attendanceLogs', JSON.stringify(logs));
        enforceHistoryLimit(); // Keep array clean
    }
    return updated;
}

// --- STUDENT SLIDER CAPTCHA LOGIC ---
...
function initSliderCaptcha() {
    ...
    img.onerror = () => {
        // Fallback abstract art if picsum fails
...
function generateAdminCaptcha() {
    ...
    // Changed to ONLY use lowercase letters (no numbers, uppercase, or symbols)
    ...
    for (let i = 0; i < captchaText.length; i++) {
        ...
        ctx.translate(x, y);
        const angle = (Math.random() * 0.8) - 0.4; // Heavy rotation
        ctx.rotate(angle);
    ...
    if (activeId) {
        ...
        if (student) {
            ...
            if (hasTimeIn && !hasTimeOut) {
                ...
                return; // Stop here, device is locked
            }
        }
        // If we reach here, the shift is no longer valid (e.g. midnight auto-timeout cleared it)
        localStorage.removeItem('activeDeviceStudent');
    }
    
    // Reset UI if no active lock
